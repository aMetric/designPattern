 ...
 */
@Component
public class DeprecatedReceiveObserver extends DeprecatedAbstarctObserver{
    ...
    public void orderStateHandle(String orderId, String orderState) {
        if(!orderState.equals("ORDER_WAIT_FINISH")){
            return;
        }

        //通过命令模式进行后续处理
        System.out.println("监听到：订单签收成功。通过命令模式进行后续处理");
    }
}
